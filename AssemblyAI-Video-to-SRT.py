...
# --- STEP 1: AUTO-INSTALLATION LOGIC ---
# List of required third-party libraries: (Package_Name, Import_Name)
REQUIRED_LIBRARIES = [
...
def install_missing_libraries():
    """Checks for missing libraries and installs them via pip."""
    for package, import_name in REQUIRED_LIBRARIES:
        # Check if the module is already installed
        spec = importlib.util.find_spec(import_name)
        if spec is None:
            print(f"Library '{package}' not found. Installing now...")
            try:
            ...
            # Library is already present, skip installation
            pass

# Run the installation check before anything else
install_missing_libraries()

# --- STEP 2: ACTUAL IMPORTS ---
# Now that we are sure libraries exist, we import them
...
# --- STEP 3: CONSTANTS AND CORE LOGIC ---
...
def get_full_language_name(code):
    """Maps ISO code to full language name."""
    code_up = code.upper()
...
def create_professional_srt(media_path, srt_path, api_key, target_lang_code, log_callback, lang_callback):
    ...
    1. Extract/Normalize audio using FFmpeg.
    2. Transcribe via AssemblyAI.
    3. Save original SRT with professional formatting.
    4. Translate if a target language is selected.
    """
    try:
        ...
        log_callback("[1/4] Normalizing audio using FFmpeg...")
        ffmpeg_cmd = "ffmpeg" 
        ...
        # Get and display detected language
        ...
        log_callback(f"Detected Source Language: {display_name}")
        ...
        with open(srt_path, 'w', encoding='utf-8') as f:
            for utterance in transcript.utterances:
                ...
                for i, word in enumerate(words):
                    ...
                    # Logic to split based on pause duration
                    if i < len(words) - 1:
                        if (words[i+1].start - word.end) / 1000 > MIN_PAUSE_BETWEEN_BLOCKS:
                            should_split = True
                    
                    # Logic to split based on text length
...
class ProfessionalApp(ctk.CTk):
    def __init__(self):
        ...
        # UI: Language Translation Selector
        ...
        # UI: Detected Info Display
        self.info_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.info_frame.grid(row=1, column=0, padx=20, pady=5, sticky="ew")
        ctk.CTkLabel(self.info_frame, text="Detected Source Language:", font=("Arial", 13, "bold")).pack(side="left", padx=5)
    ...
    def browse_file(self):
        """Opens file dialog for media selection."""
    ...
    def start_process(self):
        """Validates inputs and starts background worker thread."""
    ...
    def worker(self, p, s, k, t):
        """Thread worker to run the heavy logic."""
